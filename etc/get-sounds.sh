...
TYPES=(shot boom nuke hurt death deflect shield dirt win)
declare -A LIMIT=([shot]=6 [boom]=6 [nuke]=3 [hurt]=8 [death]=6 [deflect]=4 [shield]=3 [dirt]=4 [win]=3)
declare -A COUNT=()
...
process(){ # вход выход макс_длина пик_dB [доп_фильтр]
  ...
  ffmpeg -v error -y -i "$in" -ac 1 -af "$f" -ar 32000 "$tmp" || return 1
  ...
  mx=$(ffmpeg -hide_banner -i "$tmp" -af volumedetect -f null - 2>&1 \
        | sed -n 's/.*max_volume: \(-\{0,1\}[0-9.]*\) dB.*/\1/p' | tail -1)
  gain=$(awk -v m="${mx:-0}" -v p="$peak" 'BEGIN{printf "%.2f", p-m}')
  ffmpeg -v error -y -i "$tmp" -af "atrim=end=$len,afade=t=out:st=$fst:d=0.12,volume=${gain}dB" \
...
pick dirt    0.7 -3 "lowpass=f=2500" 'soft.*heavy|heavy|mining' k-impact
